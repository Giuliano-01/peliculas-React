...
function Resultados2(props){
    ...
    let categorie = currentParams.categorie;
    let genre = currentParams.genre;
    let streaming = currentParams.streaming;

    console.log(categorie); // get new values onchange
    console.log(genre); // get new values onchange
    console.log(streaming); // get new values onchange


    useEffect(()=>{

        //la solicitud de la api con el objeto json de la busqueda
        const endPoint = `https://api.themoviedb.org/3/discover/movie?api_key=1eef2cd243e3e41df623a92b9e33798c&language=es-ES&sort_by=${categorie}&page=1&with_genres=${genre}&with_watch_providers=${streaming}&watch_region=AR`;
    ...
    },[categorie, genre, searchParams, streaming]); //el keyword cuando cambia refresca la pagina
