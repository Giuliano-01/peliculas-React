                                    ...
                                    { 
                                        ...
                                        (<div className="card w-100 h-100 d-flex">
                                            ...
                                            <div className="card border w-100 h-100 d-flex" key={oneMovie.inx}>
                                                ...
                                                <button className='favourite-btn' onClick={props.addOrRemoveFavs} data-movie-id={oneMovie.id}>🖤</button> {/*{(favMovies !== []) && ((favMovies.find(favMovies => favMovies.id === `${oneMovie.id}`) && ("❤")))}{(favMovies === []) && (favMovies.find(favMovies => favMovies.id === `${oneMovie.id}`) && ("🖤"))}*/}
                                    ...
                                    { 
                                        ...
                                        (<div className="card w-100 h-100 d-flex">
                                            ...
                                            <div className="card border w-100 h-100 d-flex" key={oneMovie.inx}>
                                                ...
                                                <button className='favourite-btn' onClick={props.addOrRemoveFavs} data-movie-id={oneMovie.id}>🖤</button>
                                                <div className="card-body d-flex flex-column justify-content-between">
                                                  <h4 className="card-title text-center">{oneMovie.title.substring(0,30)}...</h4>
                                                  <p className="card-text">{oneMovie.overview.substring(0,100)}...</p>
                                                  <Link to={`/detalle?movieID=${oneMovie.id}`} className="btn btn-info d-flex justify-content-center">Detalle</Link>
